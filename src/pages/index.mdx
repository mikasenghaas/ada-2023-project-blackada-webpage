...
To make things more manageable (and because we love a good categorization), we've grouped the original 104 beer substyles into 11 broader style categories. 🎨 Also, disregard any reviews with less than 50 words. Why? Because, they mostly looked something like this

<Review
    review="$$$111111111111111123xxxxx023322222"
    author="Lalaland"
    beer="Scotch IPA"
    date="Jan 1st 2021"
/>

Later on our journey we will be computing consensus between reviewers. As this is only meaningful for a lot of people, we also removed all reviews of beers that had less than than 50 reviews overall - sorry to all you really niche beers out there! 🍺 After all this processing magic, we were still left with **1,65M** reviews in our bags - still heavy!

## Untangling the Taste Buds: Our Three-Step Process

---

We got many reviews, but how can we tell if the words used relate to the actual taste of the beer? Well, we can't - because do that we would need information things like the chemical composition of the beer, the taste buds of the reviewer, and the exact moment in time when the review was written. But we can do the next best thing: **we can see if people generally agree on the words they use to describe the beer**. 🤝

Following this "wisdom-of-the-crowd" approach, a descriptor can be considered meaningful if many, independent reviewers use similar descriptors for a beer's taste. To quantify consensus, we use natural language processing techniques to extract descriptors of a beer's taste and numerically represent these descriptors to compute similarity or consensus scores. The consensus scores between beer reviews will unveil whether there is a shared understanding of taste among beer geeks.
